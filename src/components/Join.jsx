...
const Join = () => {
  ...
  const idValidation = async (e) => {
    ...
    if (!regExp.test(e.target.value)) {
      ...
      const response = await axios.post("http://localhost:8085/CodeBridge/IdCheck.do", obj)
        .then((res) => {
  ...
  return (
    <div>
      <form onSubmit={JoinMember}>
        <h1>JoinPage</h1>
        <input type="email" placeholder="email" value={id} onChange={(e) => setId(e.target.value)} onBlur={idValidation} /> <br />
